...
- [RectsPatchModule](https://github.com/zugdurchfahrt/seed-noise-consistency-contracts/blob/main/16._RectsPatchModule.md)
...
Модуль влияет на измерения косвенно:

```text
не API result override,
не descriptor replacement,
не DOMRect object fabrication,
а изменение DOM/CSS inputs перед нативным layout calculation.
```

---

## 4. Что модуль не делает
...
Если эти запреты нарушены, модуль перестаёт соответствовать внешнему normative contract независимо от того, проходит ли отдельный тест.
...
Этот канал не утверждает, что конкретный элемент является именованной сущностью конкретной страницы. Он фиксирует, что текстовый glyph node может участвовать в pixel/rendering side measurements.
