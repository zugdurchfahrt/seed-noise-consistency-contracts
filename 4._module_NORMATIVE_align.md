...
1. Если `CanvasPatchContext.__logger.__DEGRADE__?.diag` существует, используем `.diag`.
2. Иначе, если `CanvasPatchContext.__logger.__DEGRADE__` callable, используем `__DEGRADE__(code, err, extra)` (shape `extra` как у `.diag`).
...
```javascript
const D = window.CanvasPatchContext?.__logger?.__DEGRADE__;
